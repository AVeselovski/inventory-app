  ...
  <div class="category__item" :class="[isDraggable && 'is-draggable']">
    <div class="flex justify-between">
      <div v-if="!isEditing" class="flex">
        <button @click="startEditing" class="btn-icon mr-2">
          <icon name="edit-small" size="18"></icon>
        </button>
        <span>{{ item.name }}</span>
      </div>
      <div v-if="!isEditing" class="flex">
        <button @click="subtract" class="btn-icon btn-icon-highlight">
          <icon name="chevron-left" size="18"></icon>
        </button>
        <span class="text-m mono mx-1" :class="[quantityClass]">x{{ item.quantity }}</span>
        <button @click="increase" class="btn-icon btn-icon-highlight">
    ...
    <form v-if="isEditing" v-on-clickaway="stopEditing" class="my-1">
      ...
      <div v-if="itemErrors && itemErrors.name">
        <span class="text-s text-danger">{{ _showError(itemErrors.name) }}</span>
      </div>
      ...
      <div v-if="itemErrors && itemErrors.quantity">
        <span class="text-s text-danger">{{ _showError(itemErrors.quantity) }}</span>
      </div>
      <div v-if="itemErrors && itemErrors.minimum">
        <span class="text-s text-danger">{{ _showError(itemErrors.minimum) }}</span>
      </div>
      <div class="flex justify-between mt-2">
        <div class="flex">
          <button @click.prevent="updateItem" class="btn btn-inverted-success btn-m mr-2" type="submit">
            Edit
          </button>
          <button @click="stopEditing" class="btn-icon btn-icon-highlight" type="button">
