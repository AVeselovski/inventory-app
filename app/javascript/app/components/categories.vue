<template>
  <draggable
    v-model="categories"
    @end="categoryMoved"
    class="categories-container drag-area"
  ...
  >
    <category
      ...
      :category="category"
      @remove-category="removeCategory(category.id, index)"
      @item-moved="itemMoved"
      :key="category.id"
    ></category>

    <div class="new-category">
      <button v-if="!isEditing" @click="startEditing" class="btn-full btn-default-dark">
        + Add Category
      </button>
...
const placeholders = ["Swords", "Armor", "Potions", "Quest Items", "Junk"];
