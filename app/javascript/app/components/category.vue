<template>
  <div class="category is-draggable">
    <div class="category__header drag-handle">
      <h4 v-if="!isEditingTitle" @click="startEditingTitle" class="heading-4 info my-1 pl-2">
        {{ category.name }}
      </h4>
      <form v-if="isEditingTitle" @submit.prevent="updateTitle">
        <input
          ...
          class="input category__title-input"
          ref="editTitleInput"
          type="text"
        />
      </form>
      <dropdown position="bottom">
        <template v-slot:trigger="slotProps">
          <button @click="slotProps.toggle" class="btn-icon btn-icon-highlight" type="button">
            <icon name="options" size="24"></icon>
          </button>
        </template>
        <template v-slot:items>
          <button @click.prevent="isDeleteModalActive = true" class="link-danger link-icon flex" href="#">
            <icon name="delete" size="18"></icon>Delete category
          </button>
        </template>
      </dropdown>
    </div>
    <div class="category__body">
      ...
      <button v-if="!isEditingNewItem" @click="startEditingNewItem" class="btn-full btn-default mt-2">
        + Add Item
      </button>
      <form v-if="isEditingNewItem" v-on-clickaway="stopEditingNewItem" class="mt-2">
        <input
          v-model="newItem"
          class="input"
          :class="[newItemErrors && newItemErrors.name && 'input-error']"
          :placeholder="getPlaceholder()"
          ref="newItemInput"
          type="text"
        />
        <div v-if="newItemErrors && newItemErrors.name">
          <span class="text-s text-danger">{{ _showError(newItemErrors.name) }}</span>
        </div>
        <div class="flex mt-2">
          <button
            @click.prevent="submitNewItem(category.id)"
            class="btn btn-success btn-m mr-2"
            type="submit"
          >
            Add
          </button>
          <button @click="stopEditingNewItem" class="btn-icon btn-icon-highlight" type="button">
            <icon name="close-small" size="18"></icon>
          </button>
        </div>
      </form>
    </div>

    <modal v-show="isDeleteModalActive" @close="isDeleteModalActive = false" dismissMsg="Nope...">
      <template v-slot:body>Delete {{ category.name }} category?</template>
      <template v-slot:footer>
        <button @click="removeCategory" class="btn btn-danger">
          Yes, delete it!
        </button>
      </template>
    </modal>
